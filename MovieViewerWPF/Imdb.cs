    ...
    public class IMDb
    {
        ...
        public void ParseIMDbPage()
        {
            ...
            if (!string.IsNullOrEmpty(Id))
            {
                ...
                Genres = matchAll(@"<a.*?>(.*?)</a>", match(@"Genre.?:(.*?)(</div>|See more)", html)).ToList();
                ...
                Poster = match(@"<a name=""poster"".*?><img.+?src=[\""'](.+?)[\""'].*?>", html);
                sw1.Restart();
        ...
        {
            sw1.Restart();
            var data = new Regex(regex, RegexOptions.Multiline).Match(html).Groups[i].Value.Trim();
            sw1.Stop();
            return data;
        }
        public static Stopwatch sw1 = new Stopwatch();
        //Match all instances and return as ArrayList
        private IEnumerable<string> matchAll(string regex, string html, int i = 1)
        {
            sw1.Restart();
            //List<string> list = new List<string>();
            foreach (Match m in new Regex(regex, RegexOptions.Multiline).Matches(html))
                yield return m.Groups[i].Value.Trim();
            sw1.Stop();
            //return list;
        }
